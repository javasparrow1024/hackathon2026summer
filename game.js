const gameEl = document.querySelector(".game");
    ...
    const enemyAvatarEl = document.querySelector("[data-enemy-avatar]");
    const enemyAvatarTextEl = document.querySelector("[data-enemy-avatar-text]");
    const enemyAvatarNameEl = document.querySelector("[data-enemy-avatar-name]");
    const restartButton = document.querySelector("[data-restart]");

    const enemyList = [
      { name: "スライム", level: 1, maxHp: 40, attackMin: 5, attackMax: 9, attackInterval: 2300, color: "#2c9be2", marker: "SLIME", image: "" },
      { name: "コボルト", level: 2, maxHp: 62, attackMin: 7, attackMax: 12, attackInterval: 2100, color: "#9f6a39", marker: "KOBOLD", image: "" },
      { name: "ゴーレム", level: 3, maxHp: 92, attackMin: 10, attackMax: 16, attackInterval: 2500, color: "#747a7f", marker: "GOLEM", image: "" },
      { name: "ドラゴン", level: 4, maxHp: 135, attackMin: 14, attackMax: 22, attackInterval: 1900, color: "#b84b3d", marker: "DRAGON", image: "" }
    ];
    ...
    const heroes = [
      ...
      },
      {
        ...
        skills: [
          ...
          { label: "紫電一閃", word: "shidenissen", kind: "attack" },
    ...
    function renderEnemyVisual() {
      ...
      enemyVisualEl.classList.toggle("has-image", hasImage);
      enemyVisualEl.setAttribute("aria-label", `${enemy.name}の${hasImage ? "グラフィック" : "仮グラフィック"}`);
      enemyAvatarEl.style.setProperty("--enemy-color", enemy.color);
      enemyAvatarEl.style.backgroundImage = hasImage ? `url("${enemy.image}")` : "none";
      enemyAvatarTextEl.textContent = enemy.marker;
      enemyAvatarNameEl.textContent = enemy.name;
    }
    ...
    function enemyAttack() {
      ...
      const damage = attack.guarded ? Math.max(2, Math.floor(baseDamage * .55)) : baseDamage;
